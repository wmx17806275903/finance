...
export class BudgetComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    var api_expense = "budget/details";
    this.httpDomain.get(api_expense).then((response) => {
      this.budget = response;
      for (var i = 0; i < this.budget.length; i++) {
        xy.push({ name: this.budget[i]["dateByMonth"], value: this.budget[i]["value"] });
        xData.push(this.budget[i]["dateByMonth"]);
        yData.push(this.budget[i]["value"]);
      }
    })
  ...
  keyUp(index) {
    ...
    this.budget[index].expenseItems.push(newData);
